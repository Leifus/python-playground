...
class BilliardsGameTable(GameTable):
    def __init__(self, size, position, space_config: GameSpaceConfig, draw_mode: DrawModeEnum):
        super(BilliardsGameTable, self).__init__(size=size, position=position, space_config=space_config, draw_mode=draw_mode)
    ...
    def setup_visuals(self):
        ...
        if not self.orig_image:
            print('BilliardsGameTable: No table img', self.rich_media_path)
            return
